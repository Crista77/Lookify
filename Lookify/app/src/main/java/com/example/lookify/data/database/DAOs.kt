...
interface Notify_Reached_DAO {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    suspend fun insert(vararg reachedNotify: Reached_Notify)
    @Query("UPDATE reached_notify SET letta = 1 WHERE id_user = :userId AND notifica_id = :notificaId")
    suspend fun markAsRead(userId: Int, notificaId: Int)
    @Query("DELETE FROM reached_notify WHERE id_user = :userId AND notifica_id = :notificationId")
    suspend fun deleteNotification(userId: Int, notificationId: Int)
}
...
interface Followers_DAO {
    ...
    @Query("DELETE FROM followers WHERE seguace_id = :followerId AND seguito_id = :followedId")
    suspend fun deleteFollower(followerId: Int, followedId: Int)
